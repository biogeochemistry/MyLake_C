...
%Lisätty erikseen näkyvä ja latentti outputtiin! PK 03.02.2016

function [Qsw,Qlw,Qsl,tau,Dayfrac,Dayfracheating,Qs,Ql] = heatflux_v12(t,GR,CC,Ta,Ra,Pa,Ua,Tw,lat,...
...
A = hfbulktc_speed(0.63*Ua,10,Ta,2,Ra,2,Pa,Tw); %function modified (speed-up) by TSA
Qsl = A(:,1) + A(:,2);
Qs = A(:,1); Ql = A(:,2);
...
Qlw = blwhf(Tw,Ta,Ra,cloudcor(CC,'bunker',lat));
    %if(t==datenum([2013 10 30]));keyboard;end
...
if((isnan(GR))|(Dayfrac==0))   % if Global radiation input is missing, or Dayfrac=0, calculate total transmissivity from formulaes 
...
Qsw = mean(Qma);
%if(t==datenum([2013 7 1]));keyboard;end
%wind stress (N/m2)
if (Ua>0)
tau=stressve(Ua, 10);
else
