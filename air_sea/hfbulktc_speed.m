...
ii = ii + 1;     %modified by TSA
if (ii>80),     %modified by TSA
 warning('Iteration did not converge (hfbulktc_speed)!');
 break          %modified by TSA; break out of the iteration loop after 80 rounds
end;            %modified by TSA
...
if nargin > 8
  ...
  % otherwise
  A=[Hs Hl Hl_webb stress U_star T_star Q_star L zetu CD CT CQ RI];
  if(abs(Ta-104.24375)<0.0001);keyboard;end
end
